...
namespace Tiandao.Collections.Test
{
    public class NamedCollectionTest
    {
		...
		public void AddTest()
		{
			...
			Assert.Equal(3, _employees.Count);
		...
		public void ClearTest()
		{
			...
			Assert.Equal(totalCount -= customerCount, _list.Count);

			var employeeCount = _employees.Count;
			_employees.Clear();

			Assert.Equal(0, _employees.Count);
			Assert.Equal(totalCount -= employeeCount, _list.Count);
