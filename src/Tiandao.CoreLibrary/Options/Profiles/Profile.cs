...
namespace Tiandao.Options.Profiles
{
	...
	/// </remarks>
#if !CORE_CLR
	public class Profile : MarshalByRefObject, IOptionProvider, Serialization.ISerializable
#else
	public class Profile : IOptionProvider, Serialization.ISerializable
#endif
	{
		#region 枚举定义
		...
		public void SetOptionObject(string path, object optionObject)
		{
			...
			else
			{
				...
				{
					section.SetEntryValue(name, Common.Converter.ConvertValue<string>(optionObject));
				}
			}
		}

		void Serialization.ISerializable.Serialize(Stream serializationStream)
		{
			...
			if(dictionary != null)
			{
				foreach(DictionaryEntry entry in dictionary)
				{
					section.SetEntryValue(
						Common.Converter.ConvertValue<string>(entry.Key),
						Common.Converter.ConvertValue<string>(entry.Value));
				}

				return;
			}

			if(Common.TypeExtension.IsAssignableFrom(typeof(IDictionary<,>), value.GetType()))
			{
				foreach(var entry in (IEnumerable)value)
				{
					...
					section.SetEntryValue(
						Common.Converter.ConvertValue<string>(entryKey),
						Common.Converter.ConvertValue<string>(entryValue));
				}
			}
		}
