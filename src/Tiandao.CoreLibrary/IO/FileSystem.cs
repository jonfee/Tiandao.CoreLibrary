...
namespace Tiandao.IO
{
    public static class FileSystem
    {
		...
		private static Services.IServiceProvider _providers;
		...
		/// </summary>
		public static Services.IServiceProvider Providers
		{
			get
			{
				if(_providers == null)
					System.Threading.Interlocked.CompareExchange(ref _providers, new Services.ServiceProvider(), null);
		...
		public static string GetUrl(string virtualPath)
		{
			...
			if(Text.TextRegular.Uri.Url.IsMatch(virtualPath))
				return virtualPath;
		...
		public class Matcher : Services.IMatcher<string>
		{
			...
			bool Services.IMatcher.Match(object target, object parameter)
			{
				var fileSystem = target as IFileSystem;
